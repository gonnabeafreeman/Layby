...
普通文件保留在原位置，Layby 保存的是文件引用。默认拖出使用复制操作，成功拖出后条目仍会留在停放区，方便再次使用。若要移动原文件，按住 **⌘⇧ Command + Shift** 开始将文件拖到 Finder 文件夹；可在设置的「拖出文件」中更换快捷键。只有目标执行移动且原路径文件确实离开后，Layby 才移除对应条目；取消或失败会保留条目。来源应用通过 File Promise 生成的临时副本不支持移动其原件。
